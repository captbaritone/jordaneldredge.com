...
function MarkdownAst({ node }) {
  switch (node.type) {
    ...
    case "paragraph":
      // The audio/video element has <div>s in it, which will cause hydration issues
      // if nested within a <p> tag, so we check here and hoist the element
      // out of the <p> tag if it is the only child.
      if (node.children.length === 1) {
        if (isAudioLink(node.children[0])) {
          return <AudioElement src={node.children[0].url} />;
        }
        if (isVideoLink(node.children[0])) {
          return <VideoElement src={node.children[0].url} />;
        }
      }
      return (
        <p>
    ...
    case "link": {
      ...
      if (isVideoLink(node)) {
        return <VideoElement src={node.url} />;
      }
    ...
    case "image":
      if (node.imageProps) {
        return (
          <ImageViewTransition id={node.url}>
            <Image
              priority={true}
              unoptimized={node.url.endsWith(".gif")}
              src={node.url}
              alt={node.alt}
...
function isVideoLink(node) {
  return (
    node.type === "link" &&
    (node.url.endsWith(".mp4") ||
      node.url.endsWith(".mov") ||
      node.url.endsWith(".webm") ||
      node.url.endsWith(".avi") ||
      node.url.endsWith(".mkv") ||
      node.url.endsWith(".m4v") ||
      node.url.endsWith(".ogv"))
  );
}
