...
export async function getTilPages(): Promise<Array<PartialBlockObjectResponse | BlockObjectResponse>> {
  const children = await notion.blocks.children.list({
    block_id: TIL_INDEX_PAGE_ID,
  });

  const childPages = children.results.filter(
...
export async function getTilPage(id: string): Promise<{ markdown: string, title: string, created_time: string }> {
  ...
  const [page, pageBlocks] = await Promise.all([
    pagePromise, blocksPromise
  ]);

  if (page.parent.type !== "page_id" || page.parent.page_id !== TIL_INDEX_PAGE_ID) {
